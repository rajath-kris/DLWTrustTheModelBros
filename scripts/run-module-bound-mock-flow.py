...
def _upload_material(
    *,
    ...
    url = f"{base_url.rstrip('/')}/api/v1/modules/{module_id}/materials"
...
def _build_capture_payload(thread_id: str, turn_index: int) -> dict[str, Any]:
    image_bytes = base64.b64decode(ONE_PIXEL_PNG_BASE64)
    return {
        "platform": "windows",
        "app_name": "Module Match Test",
        "window_title": "Module Match Test Window",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run module-bound screenshot flow (matched + unmatched)")
    parser.add_argument("--repo-root", default=str(Path(__file__).resolve().parents[1]))
    parser.add_argument("--bridge-url", default="http://127.0.0.1:8000")
    parser.add_argument("--thread-id", default="module-bound-mock-thread")
    args = parser.parse_args()

    repo_root = Path(args.repo_root).resolve()
    bridge_url = args.bridge_url.rstrip("/")

    laplace_fixture = (repo_root / "docs/mock-content/laplace_lecture_slide.md").read_text(encoding="utf-8")
    fallback_fixture = (
    ...
    fallback_module_id = "module-fallback-grounding"
    laplace_module_id = "module-laplace-transform"

    _json_request(
        bridge_url,
        "POST",
        "/api/v1/modules",
        {"module_id": fallback_module_id, "module_name": "Fallback Grounding"},
    )
    _json_request(
        bridge_url,
        "POST",
        "/api/v1/modules",
        {"module_id": laplace_module_id, "module_name": "Laplace Transform"},
    )
    ...
    laplace_material = _upload_material(
        base_url=bridge_url,
        module_id=laplace_module_id,
        material_name="Laplace lecture slide",
        filename="laplace_lecture_slide.md",
        content=laplace_fixture.encode("utf-8"),
    ...
    matched_response = _json_request(
        ...
        "POST",
        "/api/v1/captures",
        _build_capture_payload(args.thread_id, 0),
    )
    ...
    unmatched_response = _json_request(
        ...
        "POST",
        "/api/v1/captures",
        _build_capture_payload(args.thread_id, int(matched_response.get("turn_index", 0))),
    )
    ...
    summary = {
        ...
        "modules": {
            ...
            "laplace_module_id": laplace_module_id,
        },
        "materials": {
            "fallback_material_id": fallback_material.get("material_id"),
            "laplace_material_id": laplace_material.get("material_id"),
        },
        "matched_path": {
    ...
    artifact_dir = repo_root / "artifacts" / "module-bound-flow"
