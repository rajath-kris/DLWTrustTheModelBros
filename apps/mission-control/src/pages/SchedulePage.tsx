...
export function SchedulePage() {
  ...
  const { addDeadline } = useBrainState();
  ...
  const handleAddSave = useCallback(
    async (payload: { name: string; courseId: string; dueDate: string; type: string }) => {
      setSaveError(null);
      ...
      setSaving(true);
      try {
        await addDeadline(payload.courseId, {
          name,
          due_date: new Date(dueDate).toISOString(),
          readiness_score: 0,
        });
        setUserAddedDeadlines((prev) => prev.filter((item) => item.id !== extended.id));
        setAddModalOpen(false);
      } catch (error) {
        setUserAddedDeadlines((prev) => prev.filter((item) => item.id !== extended.id));
        setSaveError(error instanceof Error ? error.message : "Could not save deadline.");
      } finally {
        setSaving(false);
      }
    },
  ...
  return (
    <div className="page-shell page-fade">
      ...
      {!liveAvailable && <p className="status-line">{liveError ?? "Live data unavailable. Showing fallback schedule slices."}</p>}
      {saveError && <p className="status-line error">{saveError}</p>}
      ...
      {addModalOpen && (
        <AddDeadlineModal
          ...
          courses={courses}
          onSave={(payload) => void handleAddSave(payload)}
          saving={saving}
    ...
    <div className="schedule-modal-backdrop" onClick={onClose}>
      <div className="schedule-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <form className="schedule-modal-form" onSubmit={handleSubmit}>
          ...
          <button type="submit" className="top-bar-btn primary" disabled={saving}>
            {saving ? "Saving..." : "Save"}
          </button>
        </form>
      </div>
